...
describe('Core CLI Tests - Remove', () => {
  ...
  describe('taylored --remove (deletions patch)', () => {
    ...
    test('removes patch (without .taylored extension)', () => {
      execSync(
        `${TAYLORED_CMD_BASE} --remove ${PLUGIN_DELETIONS_NO_EXT}`,
        execOptions,
      );
      expect(
        normalizeLineEndings(
  ...
  describe('Additions Patch Tests (--remove)', () => {
    ...
    const PLUGIN_ADDITIONS_RM_FULL_PATH = path.join(
      TAYLORED_DIR_FULL_PATH,
      PLUGIN_ADDITIONS_RM_NAME,
    );
    let NEW_FILE_CONTENT_RM: string;

    const createAdditionsPatchContentRm =
      () => `diff --git a/new_file_rm.txt b/new_file_rm.txt
...
+`;
    ...
    beforeAll(async () => {
      ...
      execSync(
        `git checkout -b ${BRANCH_ADDITIONS_RM} ${initialCommitHash}`,
        execOptions,
      );
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, 'new_file_rm.txt'),
        NEW_FILE_CONTENT_RM,
      );
      execSync('git add new_file_rm.txt', execOptions);
      execSync(
        'git commit -m "Create new_file_rm.txt for additions branch rm tests"',
        execOptions,
      );
    ...
    beforeEach(async () => {
      ...
      fs.writeFileSync(
        PLUGIN_ADDITIONS_RM_FULL_PATH,
        createAdditionsPatchContentRm(),
      );
      ...
      execSync(
        `${TAYLORED_CMD_BASE} --add ${PLUGIN_ADDITIONS_RM_NAME}`,
        execOptions,
      );
      expect(
        fs.existsSync(path.join(TEST_DIR_FULL_PATH, 'new_file_rm.txt')),
      ).toBe(true); // Verify it was added
    });
    ...
    afterAll(() => {
      try {
        ...
        console.warn(
          `Warning: Could not clean up ${BRANCH_ADDITIONS_RM}. ${(e as any).message}`,
        );
      }
    });
    ...
    test('removes additions patch (with extension)', () => {
      execSync(
        `${TAYLORED_CMD_BASE} --remove ${PLUGIN_ADDITIONS_RM_NAME}`,
        execOptions,
      );
      expect(
        normalizeLineEndings(
    ...
    test('removes additions patch (without extension)', () => {
      execSync(
        `${TAYLORED_CMD_BASE} --remove ${PLUGIN_ADDITIONS_RM_NO_EXT}`,
        execOptions,
      );
      expect(
        normalizeLineEndings(
