...
const getMockTemplateSourcePath = () => {
  ...
  return path.resolve(
    ...
    isDev
      ...
      : '../../../templates/backend-in-a-box', // Should resolve to project_root/templates/...
  );
};
...
describe('handleSetupBackend', () => {
  ...
  it('should complete successfully if directory does not exist', async () => {
    ...
    expect(fs.copy).toHaveBeenCalledWith(
      getMockTemplateSourcePath(),
      path.join(mockCwd, 'taysell-server'),
    );
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining(
        'Running in test environment, using default config for .env',
      ),
    );
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining('Successfully created .env file at'),
    );
  ...
  it('should prompt for overwrite if taysell-server directory exists and user aborts', async () => {
    ...
    (fs.pathExists as jest.Mock).mockImplementation((p) => {
      ...
      if (p === path.join(mockCwd, 'taysell-server'))
        return Promise.resolve(true); // Directory exists
      return Promise.resolve(false);
    });
  ...
  it('should overwrite if taysell-server directory exists and user confirms', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining('Backend Setup Complete'),
    );
  });
...
`.trim();
  ...
  it('should exit if template source directory does not exist', async () => {
    ...
    await expect(handleSetupBackend(mockCwd)).rejects.toThrow(
      'process.exit called with 1',
    );
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining(
        'CRITICAL ERROR: Backend template source directory not found',
      ),
    );
  });
});
