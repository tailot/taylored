...
describe('Offset Functionality Tests', () => {
  ...
  describe('Offset - Deletions Patch (Simulated Step 11 from original description)', () => {
    ...
    const OFFSET_DEL_PLUGIN_FULL_PATH_S11 = path.join(
      TAYLORED_DIR_FULL_PATH,
      OFFSET_DEL_PLUGIN_NAME_S11,
    );
    ...
    beforeEach(async () => {
      ...
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, OFFSET_DEL_FILE),
        OFFSET_INITIAL_CONTENT,
      );
      execSync(`git add ${OFFSET_DEL_FILE}`, execOptions);
      execSync(
        'git commit -m "Initial commit for S11 offset test"',
        execOptions,
      );
      mainCommitForS11Patch = execSync('git rev-parse HEAD', execOptions)
        .toString()
        .trim();

      // 2. Create branch, make deletion, save patch
      execSync(
        `git checkout -b ${OFFSET_DEL_BRANCH_S11} ${mainCommitForS11Patch}`,
        execOptions,
      );
      const s11DeletedContent = OFFSET_INITIAL_CONTENT.replace(
        'Line 2: Target for deletion/modification\n',
        '',
      );
      fs.writeFileSync(
      ...
      execSync(
        'git commit -m "chore: save S11 offset patch for testing"',
        execOptions,
      );
      expect(fs.existsSync(OFFSET_DEL_PLUGIN_FULL_PATH_S11)).toBe(true);
      storedOffsetDelPluginS11Content = fs.readFileSync(
        OFFSET_DEL_PLUGIN_FULL_PATH_S11,
        'utf8',
      );
      ...
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, OFFSET_DEL_FILE),
        mainModifiedContentS11,
      );
      execSync(`git add ${OFFSET_DEL_FILE}`, execOptions);
      execSync(
        'git commit -m "Prepend lines on main to cause offset for S11"',
        execOptions,
      );
    });
    ...
    test('taylored --offset for deletions patch fails as expected', () => {
      ...
      try {
        execSync(
          `${TAYLORED_CMD_BASE} --offset ${OFFSET_DEL_PLUGIN_NAME_S11}`,
          execOptions,
        );
      } catch (error) {
        failed = true;
      ...
      expect(
        normalizeLineEndings(
          fs.readFileSync(OFFSET_DEL_PLUGIN_FULL_PATH_S11, 'utf8'),
        ),
      ).toBe(normalizeLineEndings(storedOffsetDelPluginS11Content));
      ...
      ).toBe(normalizeLineEndings(mainModifiedContentS11));
    });
  });
  ...
  describe('Offset - Additions Patch (Simulated Step 12 from original description)', () => {
    ...
    const OFFSET_ADD_PLUGIN_FULL_PATH_S12 = path.join(
      TAYLORED_DIR_FULL_PATH,
      OFFSET_ADD_PLUGIN_NAME_S12,
    );
    ...
    beforeEach(async () => {
      ...
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, OFFSET_ADD_FILE_S12),
        OFFSET_INITIAL_CONTENT,
      );
      execSync(`git add ${OFFSET_ADD_FILE_S12}`, execOptions);
      execSync(
        'git commit -m "Initial commit for S12 offset test"',
        execOptions,
      );
      mainCommitForS12Patch = execSync('git rev-parse HEAD', execOptions)
        .toString()
        .trim();

      execSync(
        `git checkout -b ${OFFSET_ADD_BRANCH_S12} ${mainCommitForS12Patch}`,
        execOptions,
      );
      ...
      execSync(`git add ${TAYLORED_DIR_FULL_PATH}`, execOptions);
      execSync(
        'git commit -m "chore: save S12 offset patch for testing"',
        execOptions,
      );
      expect(fs.existsSync(OFFSET_ADD_PLUGIN_FULL_PATH_S12)).toBe(true);
      storedOffsetAddPluginS12Content = fs.readFileSync(
        OFFSET_ADD_PLUGIN_FULL_PATH_S12,
        'utf8',
      );
      ...
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, OFFSET_ADD_FILE_S12),
        mainModifiedContentS12,
      );
      execSync(`git add ${OFFSET_ADD_FILE_S12}`, execOptions);
      execSync(
        'git commit -m "Prepend lines on main to cause offset for S12"',
        execOptions,
      );
    });
    ...
    test('taylored --offset for additions patch fails as expected', () => {
      ...
      try {
        execSync(
          `${TAYLORED_CMD_BASE} --offset ${OFFSET_ADD_PLUGIN_NAME_S12}`,
          execOptions,
        );
      } catch (error) {
        failed = true;
      ...
      expect(
        normalizeLineEndings(
          fs.readFileSync(OFFSET_ADD_PLUGIN_FULL_PATH_S12, 'utf8'),
        ),
      ).toBe(normalizeLineEndings(storedOffsetAddPluginS12Content));
      expect(
        normalizeLineEndings(
          fs.readFileSync(
            path.join(TEST_DIR_FULL_PATH, OFFSET_ADD_FILE_S12),
  ...
  describe('Offset - Successful Update', () => {
    ...
    beforeEach(async () => {
      ...
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, OFFSET_SUCCESS_FILE),
        OFFSET_INITIAL_CONTENT,
      );
      execSync(`git add ${OFFSET_SUCCESS_FILE}`, execOptions);
      execSync(
        'git commit -m "Initial commit for offset success test"',
        execOptions,
      );

      execSync(`git checkout -b ${TEMP_BRANCH}`, execOptions);
      const modifiedContent =
        OFFSET_INITIAL_CONTENT + 'Line 6: A new purely added line.\n';
      fs.writeFileSync(
        path.join(TEST_DIR_FULL_PATH, OFFSET_SUCCESS_FILE),
        modifiedContent,
      );
      ...
      fs.renameSync(
        path.join(TAYLORED_DIR_FULL_PATH, `${TEMP_BRANCH}.taylored`),
        PATCH_FULL_PATH,
      );
      ...
      execSync(`git add ${OFFSET_SUCCESS_FILE}`, execOptions);
      execSync(
        'git commit -m "Prepend lines on main to cause offset"',
        execOptions,
      );
    });
    ...
    test('should successfully update offsets after prepending lines', () => {
      ...
      execSync(
        `${TAYLORED_CMD_BASE} --offset ${PATCH_TO_OFFSET} main`,
        execOptions,
      );
      ...
      expect(updatedHunks[0].originalHeaderLine).not.toEqual(
        originalHunks[0].originalHeaderLine,
      );
      ...
      const expectedContentAfterApply =
        PREPEND_CONTENT +
        OFFSET_INITIAL_CONTENT +
        'Line 6: A new purely added line.\n';
      expect(normalizeLineEndings(finalContent)).toBe(
        normalizeLineEndings(expectedContentAfterApply),
      );
    });
  });
});
