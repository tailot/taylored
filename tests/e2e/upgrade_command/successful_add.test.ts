...
describe('taylored --upgrade successful addition scenario', () => {
  ...
  beforeEach(() => {
    ...
    fs.copyFileSync(
      path.join(FIXTURES_DIR, 'patch_add.patch'),
      path.join(tempTestDir, '.taylored', newTayloredPatchName),
    );
    ...
    fs.copyFileSync(
      path.join(FIXTURES_DIR, 'target_current_add.txt'),
      path.join(tempTestDir, 'target_current_add.txt'),
    );
    ...
    execSync(
      'git commit -m "Initial commit for upgrade test - target file"',
      execOptions,
    );

    // The patch file (now as .taylored) should also be committed to simulate a managed taylored file
    execSync(
      `git add "${path.relative(tempTestDir, tempPatchPath)}"`,
      execOptions,
    ); // Add .taylored/patch_add.taylored
    execSync(
      'git commit -m "Add taylored patch file for upgrade test"',
      execOptions,
    );
  });
  ...
  it('should correctly upgrade the patch file when frames are intact', () => {
    ...
    try {
      ...
      console.error(
        'Error executing taylored --upgrade:',
        cliOutput,
        cliErrorOutput,
      );
...
`; // Note: The final newline is important in patches
    ...
    const originalFixturePatchContent = fs.readFileSync(
      path.join(FIXTURES_DIR, 'patch_add.patch'),
      'utf8',
    );
    const backupContent = fs.readFileSync(backupPatchPath, 'utf8');
    expect(backupContent.replace(/\r\n/g, '\n')).toBe(
      originalFixturePatchContent.replace(/\r\n/g, '\n'),
    );
  });
});
