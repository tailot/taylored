...
// Line 6\n`; // Added trailing newline
        ...
        test('Multiple Blocks: Correctly extracts all blocks', () => {
            ...
            const content1 = fs.readFileSync(
                path.join(testRepoPath, TAYLORED_DIR_NAME, `1${TAYLORED_FILE_EXTENSION}`),
                'utf8'
            );
            ...
            const content2 = fs.readFileSync(
                path.join(testRepoPath, TAYLORED_DIR_NAME, `2${TAYLORED_FILE_EXTENSION}`),
                'utf8'
            );
            ...
            const content3 = fs.readFileSync(
                path.join(testRepoPath, TAYLORED_DIR_NAME, `3${TAYLORED_FILE_EXTENSION}`),
                'utf8'
            );
    ...
    describe('Exclude Functionality', () => {
        ...
        test('Correctly excludes specified directories and their subdirectories', () => {
            ...
            const relevantStderr = result.stderr
                .split('\n')
                .filter((line) => !line.startsWith('Processing block') && !line.startsWith('Successfully created'))
                .join('\n');
            ...
            expect(fs.readFileSync(path.join(testRepoPath, 'excluded_dir1', 'file2.js'), 'utf8')).toContain(
                '// <taylored number="2">'
            );
            expect(
                fs.readFileSync(path.join(testRepoPath, 'excluded_dir1', 'sub_excluded_dir', 'file_in_sub.js'), 'utf8')
            ).toContain('// <taylored number="5">');
            expect(fs.readFileSync(path.join(testRepoPath, 'not_excluded_dir', 'file3.js'), 'utf8')).toContain(
                '// <taylored number="3">'
            );
            expect(fs.readFileSync(path.join(testRepoPath, 'excluded_dir2', 'file4.js'), 'utf8')).toContain(
                '// <taylored number="4">'
            );
