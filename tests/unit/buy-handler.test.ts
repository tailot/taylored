...
describe('handleBuyCommand', () => {
  ...
  const validTaysellFileContent: taysellUtils.TaysellFile = {
    ...
    sellerInfo: {
      ...
      contact: 'contact@seller.com',
    },
    metadata: {
      name: 'My Test Patch',
      description: 'A patch for testing',
  ...
  beforeEach(() => {
    ...
    jest
      .mocked(fs.readFile)
      .mockImplementation(async () => JSON.stringify(validTaysellFileContent));
      ...
      if (typeof callback === 'function') {
        ...
        process.nextTick(() => {
          if ((options as https.RequestOptions).method === 'POST') {
            (mockResForRequest as PassThrough).emit(
              'data',
              Buffer.from('mocked patch content'),
            );
          }
    ...
    jest
      .mocked(https.request)
      .mockImplementation(mockRequestFn as typeof https.request);
  });
  ...
  it('should complete successfully for a valid .taysell file and successful purchase', async () => {
    ...
    mockHttpsGetResponse.on.mockImplementation(function (
      this: any,
      event: string,
      cb: (...args: any[]) => void,
    ) {
    ...
    expect(open).toHaveBeenCalledWith(
      expect.stringContaining(
        validTaysellFileContent.endpoints.initiatePaymentUrl,
      ),
    );
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(
      path.join(
        mockCwd,
        TAYLORED_DIR_NAME,
        `test_patch_id_123${TAYLORED_FILE_EXTENSION}`,
      ), // Corrected filename
  ...
  it('should exit if .taysell file is not found', async () => {
    ...
    await expect(
      handleBuyCommand(mockTaysellFileName, false, mockCwd),
    ).rejects.toThrow('process.exit called');
    expect(utils.printUsageAndExit).toHaveBeenCalledWith(
      expect.stringContaining('Taysell file not found'),
    );
  });
});
