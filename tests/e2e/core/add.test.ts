    ...
    describe('taylored --add (deletions patch)', () => {
        test('adds patch (with .taylored extension)', () => {
            ...
            expect(normalizeLineEndings(fs.readFileSync(path.join(TEST_DIR_FULL_PATH, 'file1.txt'), 'utf8'))).toBe(
                normalizeLineEndings(MODIFIED_FILE1_DELETIONS_CONTENT)
            );
...
+`;
        ...
        test('taylored --add (additions patch, with extension)', () => {
            ...
            expect(normalizeLineEndings(fs.readFileSync(path.join(TEST_DIR_FULL_PATH, 'new_file.txt'), 'utf8'))).toBe(
                normalizeLineEndings(NEW_FILE_CONTENT)
            );
        });
