...
async function updatePatchOffsets(patchFilePathArg, repositoryPathArg) {
    ...
    try {
        ...
        if (hasForwardChanges) {
            ...
            await execGit(repoRoot, ['commit', '-m', "Temporary: Applied patch (forwards) for offset update", '--quiet']);
            ...
            if (applyReverseResult.success) {
                ...
                if (hasReverseChanges) {
                    ...
                    await execGit(repoRoot, ['commit', '-m', "Temporary: Applied patch (backwards) to get a non-empty diff", '--quiet']);
                    ...
                    await execGit(repoRoot, ['commit', '--allow-empty', '-m', "Temporary: Patch produced no diff (neither forwards nor backwards)", '--quiet']);
                ...
                await execGit(repoRoot, ['commit', '--allow-empty', '-m', "Temporary: Failed 'apply -R'; input patch not reversible", '--quiet']);
