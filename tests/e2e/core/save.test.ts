...
describe('Core CLI Tests - Save', () => {
    ...
    test('taylored --save correctly creates a patch from a branch (purely additive)', () => {
        ...
        try {
            ...
            execSync(`git checkout -b ${BRANCH_SAVE_TEST} ${initialCommitHash}`, execOptions);
            fs.writeFileSync(path.join(TEST_DIR_FULL_PATH, FILE_FOR_SAVE_TEST), FILE_FOR_SAVE_TEST_CONTENT);
            ...
            execSync('git commit -m "Commit for pure save test - new file only"', execOptions);
            ...
            expect(patchContent).toContain(`+${FILE_FOR_SAVE_TEST_CONTENT.split('\n')[0]}`); // Check for new file content
