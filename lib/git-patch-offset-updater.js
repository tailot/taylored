...
function extractMessageFromPatch(patchContent) {
    ...
    for (const line of lines) {
        ...
        if (!inHeader && !foundDiff && line.trim() !== '' && !line.startsWith('From ') && !line.startsWith('Date: ')) {
            // Avoid lines that look like patch metadata
    ...
    if (potentialMessageLines.length > 0) {
        // Take the first non-empty line as a potential subject
        const firstMeaningfulLine = potentialMessageLines.find(l => l !== '');
