...
describe('validateTaysellFileContent', () => {
  ...
  it('should throw if sellerInfo.name is missing', () => {
    const data = {
      ...validTaysellData,
      sellerInfo: { ...validTaysellData.sellerInfo, name: undefined as any },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
  ...
  it('should throw if metadata.name is missing', () => {
    const data = {
      ...validTaysellData,
      metadata: { ...validTaysellData.metadata, name: undefined as any },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
  ...
  it('should throw if endpoints.getPatchUrl is not HTTPS', () => {
    const data = {
      ...validTaysellData,
      endpoints: {
        ...validTaysellData.endpoints,
        getPatchUrl: 'http://example.com/api/get-patch',
      },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
  ...
  it('should throw if endpoints.initiatePaymentUrl is an invalid URL', () => {
    const data = {
      ...
      endpoints: {
        ...validTaysellData.endpoints,
        initiatePaymentUrl: 'not-a-url',
      },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
      /one of the endpoint URLs is invalid/,
    );
  });
  ...
  it('should throw if endpoints.getPatchUrl is an invalid URL', () => {
    const data = {
      ...validTaysellData,
      endpoints: { ...validTaysellData.endpoints, getPatchUrl: 'not-a-url' },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
      /one of the endpoint URLs is invalid/,
    );
  ...
  it('should throw for invalid payment.currency format', () => {
    const data = {
      ...validTaysellData,
      payment: { ...validTaysellData.payment, currency: 'USDOLLARS' },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
  ...
  it('should throw if payment.price is missing', () => {
    const data = {
      ...validTaysellData,
      payment: { ...validTaysellData.payment, price: undefined as any },
    };
    expect(() => validateTaysellFileContent(data)).toThrow(
      'Invalid .taysell file: payment.price must be a string and payment.currency must be a 3-letter string.',
    );
