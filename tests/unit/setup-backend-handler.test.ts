...
describe('handleSetupBackend', () => {
    ...
    beforeEach(() => {
        ...
        // REMOVE inquirer.prompt mock from beforeEach - tests will handle it.
    });
    ...
    it('should complete successfully if Docker is installed and directory does not exist', async () => {
        ...
        jest.mocked(inquirer.prompt).mockResolvedValueOnce({
            ...
            patchEncryptionKey: 'test-encryption-key-12345678901234567890',
            serverPort: '3001',
        });
        ...
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('.env file created at')); // More specific check
    ...
    it('should prompt for overwrite if taysell-server directory exists and user aborts', async () => {
        ...
        jest.mocked(inquirer.prompt) // Corrected mock
            .mockResolvedValueOnce({ overwrite: false }) // First call for overwrite
            .mockResolvedValueOnce({ /* env answers */ }); // This won't be reached if aborting

        await handleSetupBackend(mockCwd);

        expect(jest.mocked(inquirer.prompt)).toHaveBeenCalledWith(expect.arrayContaining([ // Corrected mock
    ...
    it('should write correct .env content based on user input', async () => {
        ...
        jest.mocked(inquirer.prompt).mockResolvedValue(mockAnswers); // Corrected mock
