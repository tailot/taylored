...
describe('validateTaysellFileContent', () => {
    const validTaysellData: TaysellFile = {
        ...
        endpoints: { initiatePaymentUrl: 'https://example.com/pay/test-patch-123', getPatchUrl: 'https://example.com/api/get-patch' },
    ...
    it('should throw if data is not an object', () => {
        expect(() => validateTaysellFileContent('not-an-object')).toThrow('Invalid .taysell file: content is not an object.');
    });
    ...
    requiredFields.forEach(field => {
        it(`should throw if required field "${field}" is missing`, () => {
            ...
            expect(() => validateTaysellFileContent(data)).toThrow(`Invalid .taysell file: missing required field "${field}".`);
        });
    });

    it('should throw for incorrect taysellVersion', () => {
        const data = { ...validTaysellData, taysellVersion: '0.9-beta' };
        expect(() => validateTaysellFileContent(data)).toThrow('Invalid .taysell file: unsupported taysellVersion "0.9-beta". Expected "1.0-decentralized".');
    });

    it('should throw for empty patchId', () => {
        const data = { ...validTaysellData, patchId: ' ' };
        expect(() => validateTaysellFileContent(data)).toThrow('Invalid .taysell file: patchId must be a non-empty string.');
    });
    ...
    it('should throw if sellerInfo.name is missing', () => {
        ...
        expect(() => validateTaysellFileContent(data)).toThrow('Invalid .taysell file: sellerInfo.name is missing or not a string.');
    });
    ...
    it('should throw if metadata.name is missing', () => {
        ...
        expect(() => validateTaysellFileContent(data)).toThrow('Invalid .taysell file: metadata.name is missing or not a string.');
    });

    it('should throw if endpoints.getPatchUrl is not HTTPS', () => {
        const data = { ...validTaysellData, endpoints: { ...validTaysellData.endpoints, getPatchUrl: 'http://example.com/api/get-patch' } };
        expect(() => validateTaysellFileContent(data)).toThrow('Invalid .taysell file: endpoints.getPatchUrl must use HTTPS.');
    });

    it('should throw if endpoints.initiatePaymentUrl is an invalid URL', () => {
        const data = { ...validTaysellData, endpoints: { ...validTaysellData.endpoints, initiatePaymentUrl: 'not-a-url' } };
    ...
    it('should throw for invalid payment.currency format', () => {
        ...
        expect(() => validateTaysellFileContent(data)).toThrow('Invalid .taysell file: payment.price must be a string and payment.currency must be a 3-letter string.');
    });
