...
const execCli = (
  args: string,
  options?: ExecSyncOptions & { cwd?: string; env?: NodeJS.ProcessEnv },
) => {
  const command = `npx ts-node ${CLI_ENTRY} ${args}`;
  const combinedEnv = { ...process.env, ...options?.env };
  return execSync(command, {
    stdio: 'pipe', // Capture output, suppress in tests unless debugging
    ...options,
    env: combinedEnv, // Pass combined environment
...
describe('Taysell E2E Tests', () => {
  ...
  describe('setup-backend', () => {
    it('should create taysell-server directory with template files', () => {
      const testEnv = {
        ...process.env,
        TAYLORED_E2E_SKIP_DOCKER_CHECK: 'true',
      };
      ...
      try {
        ...
        output = execSync(command, {
          cwd: currentTempDir,
          stdio: 'pipe',
          env: testEnv, // Pass the environment with the skip flag
        }).toString();
      } catch (error: any) {
        ...
        console.warn(
          'setup-backend E2E command execution warning (may be due to interactivity):',
          output,
        );
      }

      expect(
        fs.pathExistsSync(path.join(currentTempDir, 'taysell-server')),
      ).toBe(true);
      expect(
        fs.pathExistsSync(
          path.join(currentTempDir, 'taysell-server', 'Dockerfile'),
        ),
      ).toBe(true);
      expect(
        fs.pathExistsSync(
          path.join(currentTempDir, 'taysell-server', 'docker-compose.yml'),
        ),
      ).toBe(true);
      expect(
        fs.pathExistsSync(
          path.join(currentTempDir, 'taysell-server', 'package.json'),
        ),
      ).toBe(true);
      expect(
        fs.pathExistsSync(
          path.join(currentTempDir, 'taysell-server', 'index.js'),
        ),
      ).toBe(true);
...
`;
    ...
    beforeEach(async () => {
      dummyTayloredFilePath = path.join(
        currentTempDir,
        `${dummyPatchName}${TAYLORED_FILE_EXTENSION}`,
      );
    ...
    it('should create encrypted patch and .taysell metadata file', async () => {
      ...
      const encryptedFilePath = path.join(
        currentTempDir,
        `${dummyPatchName}${TAYLORED_FILE_EXTENSION}.enc`,
      );
      const metadataFilePath = path.join(
        currentTempDir,
        `${dummyPatchName}.taysell`,
      );
      ...
      expect(taysellJson.endpoints.initiatePaymentUrl).toBe(
        `${envServerUrl}/pay/${taysellJson.patchId}`,
      );
      expect(taysellJson.endpoints.getPatchUrl).toBe(
        `${envServerUrl}/get-patch`,
      );
      ...
      const decryptedContent = decryptAES256GCM(
        encryptedContent,
        actualEncryptionKey,
      );
