...
const parseDiffModule = require('parse-diff'); // Added for parsing patch content
...
async function updatePatchOffsets(patchFilePathArg, repositoryPathArg) {
    ...
    try {
        ...
        // Instead of `git add .`, parse the input patch and add only the affected files.
        // This prevents unrelated untracked files from being included in the temporary commit.
        const originalPatchContent = fs.readFileSync(absolutePatchFilePath, 'utf8');
        const parsedOriginalPatch = parseDiffModule(originalPatchContent); // Changed this line
        const filesToStage = new Set();

        for (const fileDiff of parsedOriginalPatch) {
            // Add the 'from' path if it's a real file path (not /dev/null)
            if (fileDiff.from && fileDiff.from !== '/dev/null') {
                filesToStage.add(fileDiff.from);
            }
            // Add the 'to' path if it's a real file path (not /dev/null)
            // This covers additions, modifications, and renames.
        ...
        if (hasForwardChanges) {
            ...
            if (applyReverseResult.success) {
                // Similar to the forward case, stage only files affected by the reverse apply.
                // The files affected by reverse apply are the same as by forward apply.
                // The `filesToStage` set is already populated with them.
                if (filesToStage.size > 0) {
                    const filesArray = Array.from(filesToStage);
                    console.log(`Staging specific files affected by reverse patch application: ${filesArray.join(', ')}`);
                    // It's crucial that `git add` is run after `git apply -R` has modified the working tree.
                    // The paths in filesToStage are correct for identifying which files' states need to be recorded.
                    await execGit(repoRoot, ['add', ...filesArray]);
                } else {
                    console.log("No files to stage based on patch content analysis for reverse apply (original patch might be empty).");
                }

                // Alternative if `git apply -R` might affect different files (unlikely for simple patches):
                // Re-calculate filesToStage based on `git status --porcelain` after `git apply -R`
                // However, using the original patch's file list is generally safer to ensure
                // we are only committing what the original patch intended to change, just in reverse.
                // The current `filesToStage` (derived from the original patch) should be correct.
