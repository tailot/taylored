...
export const TEST_DIR_FULL_PATH = path.join(PROJECT_ROOT_PATH, TEST_SUBDIR_NAME);
export const TAYLORED_CMD_BASE = `npx ts-node ${path.join(PROJECT_ROOT_PATH, 'index.ts')}`;
export const TAYLORED_DIR_NAME = '.taylored';
export const TAYLORED_DIR_FULL_PATH = path.join(TEST_DIR_FULL_PATH, TAYLORED_DIR_NAME);

export const BRANCH_DELETIONS = 'deletions-branch';
export const PLUGIN_DELETIONS_NAME = `${BRANCH_DELETIONS}.taylored`;
export const PLUGIN_DELETIONS_FULL_PATH = path.join(TAYLORED_DIR_FULL_PATH, PLUGIN_DELETIONS_NAME);
...
export const initializeTestEnvironment = async () => {
    ...
    if (!fs.existsSync(PLUGIN_DELETIONS_FULL_PATH)) {
        throw new Error(`Failed to create plugin file ${PLUGIN_DELETIONS_FULL_PATH} in initializeTestEnvironment`);
    }
    console.log(
